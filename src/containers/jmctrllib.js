export const jmctrllib = (function() {
  //Buffers
  const gb = {
    ...
    tilt: 0, //initial 1g
    yaw_m0: [0, 0],
    yaw_m1: [0, 0], //yaw and diff yaw,m1 means former value
    pitch_m0: [0, 0],
    pitch_m1: [0, 0], //pitch and diff pitch,,m1 means former value
    yaw_swing_cnt: 0,
    yaw_swing_fep_sign: 0,
    yaw_swing_fep_cnt: -100,
    yaw_swing_seq_cnt: 1,
    yaw_swing_in_seq: 0,
    yaw_swing_start_sign: 0,
    pitch_swing_cnt: 0,
    pitch_swing_fep_sign: 0,
    pitch_swing_fep_cnt: -100,
    pitch_swing_seq_cnt: 1,
    pitch_swing_in_seq: 0,
    pitch_swing_start_sign: 0,
    em_long_cnt: 0,
    em_long_fep_cnt: -100,
    em_long_fep_sign: 0,
    em_long_seq_cnt: 1,
    em_long_in_seq: 0,
    em_long_start_sign: 0,
    em_lat_cnt: 0,
    em_lat_fep_cnt: -100,
    em_lat_fep_sign: 0,
    em_lat_seq_cnt: 1,
    ...
    em_lat_start_sign: 0,
    yawm1: 0,
    rotation: 0, //only for getSequentialYaw
  };
  //Parameters
  const param = {
    yaw_d1_th: 3.1,
  ...
  return {
    ...
    getSequentialYaw(data) {
      // eslint-disable-next-line no-unused-vars
      const yaw_temp = data.yaw + 360 * gb.rotation;
      gb.rotation +=
        Math.abs(data.yaw - gb.yawm1) > 300
          ? -1 * Math.sign(data.yaw - gb.yawm1)
          : 0;
      gb.yawm1 = data.yaw;

      return data.yaw + 360 * gb.rotation;
    },
    getSequentialSwing(data) {
      gb.rt_cnt_sw += 1;
      gb.yaw_m0[0] = data.yaw;
      if (Math.abs(gb.yaw_m1[0] - gb.yaw_m0[0]) > 300) {
        //in case one turn
        gb.yaw_m0[1] =
          gb.yaw_m1[0] -
          (gb.yaw_m0[0] + 360 * Math.sign(gb.yaw_m1[0] - gb.yaw_m0[0]));
      } else {
        gb.yaw_m0[1] = gb.yaw_m1[0] - gb.yaw_m0[0];
      }
      //in case a peak finished
      if (
        (Math.abs(gb.yaw_m1[1]) >= param.yaw_d1_th &&
          Math.abs(gb.yaw_m0[1]) >= param.yaw_d1_th &&
          Math.sign(gb.yaw_m1[1] * gb.yaw_m0[1]) == -1) ||
        (Math.abs(gb.yaw_m1[1]) >= param.yaw_d1_th &&
          Math.abs(gb.yaw_m0[1]) < param.yaw_d1_th &&
          gb.rt_cnt_sw > 10)
      ) {
      ...
      if (
        gb.yaw_swing_in_seq == 1 &&
        gb.rt_cnt_sw - gb.yaw_swing_fep_cnt > param.yaw_seq_th
      ) {
        const direction =
          -1 * gb.yaw_swing_fep_sign * ((gb.yaw_swing_seq_cnt % 2) * 2 - 1);
        const event_sw_lat = new CustomEvent('jmctrllib_swing_lat', {
          detail: { direction, count: gb.yaw_swing_seq_cnt },
        });
      ...
      if (Math.abs(gb.pitch_m1[0] - gb.pitch_m0[0]) > 300) {
        //in case one turn
        gb.pitch_m0[1] =
          gb.pitch_m1[0] -
          (gb.pitch_m0[0] + 360 * Math.sign(gb.pitch_m1[0] - gb.pitch_m0[0]));
      } else {
      ...
      //in case a peak finished
      if (
        (Math.abs(gb.pitch_m1[1]) >= param.pitch_d1_th &&
          Math.abs(gb.pitch_m0[1]) >= param.pitch_d1_th &&
          Math.sign(gb.pitch_m1[1] * gb.pitch_m0[1]) == -1) ||
        (Math.abs(gb.pitch_m1[1]) >= param.pitch_d1_th &&
          Math.abs(gb.pitch_m0[1]) < param.pitch_d1_th &&
          gb.rt_cnt_sw > 10)
      ) {
      ...
      //finish seqencial peak detection
      if (
        gb.pitch_swing_in_seq == 1 &&
        gb.rt_cnt_sw - gb.pitch_swing_fep_cnt > param.pitch_seq_th
      ) {
        const direction =
          gb.pitch_swing_fep_sign * ((gb.pitch_swing_seq_cnt % 2) * 2 - 1);
        const event_sw_long = new CustomEvent('jmctrllib_swing_long', {
          detail: { direction, count: gb.pitch_swing_seq_cnt },
        });
    ...
    getSequentialEyeMove(data) {
      ...
      if (
        gb.em_lat_in_seq == 1 &&
        gb.rt_cnt_em - gb.em_lat_fep_cnt > param.em_lat_seq_th
      ) {
        const direction =
          gb.em_lat_fep_sign * ((gb.em_lat_seq_cnt % 2) * 2 - 1);
        const event_em_lat = new CustomEvent('jmctrllib_eyemove_lat', {
          detail: { direction, count: gb.em_lat_seq_cnt },
        });
      ...
      if (
        gb.em_long_in_seq == 1 &&
        gb.rt_cnt_em - gb.em_long_fep_cnt > param.em_long_seq_th
      ) {
        const direction =
          gb.em_long_fep_sign * ((gb.em_long_seq_cnt % 2) * 2 - 1);
        const event_em_long = new CustomEvent('jmctrllib_eyemove_long', {
          detail: { direction, count: gb.em_long_seq_cnt },
        });
